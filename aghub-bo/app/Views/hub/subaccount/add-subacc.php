        ...
        <article class="card-text p-3">
            ...
            <dl class="row mb-0">
                <dd class="col-xl-5 col-lg-5 col-md-12 col-12">
                    <?=form_open('',['class'=>'form-validation addHubSubAccForm', 'novalidate'=>'novalidate']);?>
                    <div class="mb-3 row">
...
document.addEventListener('DOMContentLoaded', (event) => {
    $('.addHubSubAccForm').on('submit', function(e) {
        e.preventDefault();

        if (this.checkValidity() !== false) {
            generalLoading();

            $('.addHubSubAccForm [type=submit]').prop('disabled',true);
            ...
            .done(function() {
                $('.addHubSubAccForm [type=submit]').prop('disabled',false);
            })
            .fail(function() {
                swal.fire("Error!", "Oopss! There are something wrong. Please try again later.", "error");
            });

            $('.addHubSubAccForm').removeClass('was-validated');
            $('.addHubSubAccForm').trigger('reset');
        }
    });
});
</script>