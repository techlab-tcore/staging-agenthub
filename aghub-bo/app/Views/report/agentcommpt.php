        ...
        <article class="card-text p-3">
            ...
            <div class="col-xl-auto col-lg-auto col-md-auto col-12">
                <div class="input-group">
                    ...
                    <input type="text" class="form-control bg-white" name="start" readonly>
                </div>
            </div>
            <div class="col-xl-auto col-lg-auto col-md-auto col-12">
                <div class="input-group">
                    <span class="input-group-text"><?=lang('Input.to');?></span>
                    <input type="text" class="form-control bg-white" name="end" readonly>
                </div>
            </div>
            <div class="col-xl-auto col-lg-auto col-md-auto col-12">
                <div class="input-group">
            ...
            <table id="agcommptTable" class="w-100 nowrap table table-sm table-bordered table-hover">
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    agcommptTable(fromdate,todate,settledate,'<?=$parent;?>');
...
function agcommptTable(from,to,settledate,parent)
{
    const agcommptTable = $('#agcommptTable').DataTable({
        ...
        ajax: {
            type : "POST",
            url: '/list/report/agent-commission-pt',
            data: {"start": from, "end": to, "date":settledate , "parent": parent},
        ...
        //     // var totalOverAllPage = api.column(3).data().reduce(function(a, b){ return intVal(a) + intVal(b); }, 0);
        //     var totalOverPage = api.column(3, {page: 'current'}).data().reduce(function(a, b){ return intVal(a) + intVal(b); }, 0);
        //     $(api.column(3).footer()).html(parseFloat(totalOverPage).toFixed(5).replace(/(\.\d{4})\d*/, "$1").replace(/(\d)(?=(\d{5})+\b)/g, "$1,"));

        //     var totalOverPage2 = api.column(4, {page: 'current'}).data().reduce(function(a, b){ return intVal(a) + intVal(b); }, 0);
        //     $(api.column(4).footer()).html(parseFloat(totalOverPage2).toFixed(5).replace(/(\.\d{4})\d*/, "$1").replace(/(\d)(?=(\d{5})+\b)/g, "$1,"));

        //     var totalOverPage3 = api.column(5, {page: 'current'}).data().reduce(function(a, b){ return intVal(a) + intVal(b); }, 0);
        //     $(api.column(5).footer()).html(parseFloat(totalOverPage3).toFixed(5).replace(/(\.\d{4})\d*/, "$1").replace(/(\d)(?=(\d{5})+\b)/g, "$1,"));
        // },
        fnInitComplete: function(oSettings, json) {
            
        },
        drawCallback: function(oSettings, json) {
            $('#agcommptTable tbody tr').find('td').not('td:first-child,td:nth-child(2),td:nth-child(3)').addClass('text-end');
            $('#agcommptTable tbody tr td.dataTables_empty').removeClass('text-end');
        },
        // aoColumnDefs: [{
        //     aTargets: [3,4,5],
...
function loadparent()
{
    ...
    agcommptTable(start,end,settledate,'<?=$parent;?>');
