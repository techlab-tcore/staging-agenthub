...
function psTable(settledate,parent)
{
    const psTable = $('#psTable').DataTable({
        ...
        drawCallback: function(oSettings, json) {
            // $('#compTable tbody tr').find('td').not('td:first-child,td:nth-child(2),td:nth-child(3),td:nth-child(4)').addClass('text-end');
        ...
        aoColumnDefs: [{
            ...
            fnCreatedCell: function (nTd, sData, oData, iRow, iCol) {
                parseFloat(sData) < 0 ? $(nTd).addClass('text-danger') : $(nTd).addClass('text-success');
            }
        }]
    });

    $('#psTable tbody').on('click', 'td.details-control', function() {
        ...
        if(row.child.isShown()) {
            ...
            // Open this row
            row.child(formatAg(row.data())).show();
            tr.addClass('shown');
...
function compTable(settledate,parent)
{
    ...
    $('#compTable tbody').on('click', 'td.details-control', function() {
        var tr = $(this).closest('tr');
...
function formatAg(d)
{
    ...
    var tbl='', res='', game='', value='', value2='', value3='', value4='', value5='';
    obj.forEach(function(item, index) {
        if( index % 5 === 0 ) {
            game = '<td>' + item + '</td>';
        } else {
            value4 = (parseFloat(item) < 0 ? '<td class="text-danger">' + parseFloat(item).toFixed(2) + '</td>' :  parseFloat(item) > 0 ? '<td class="text-success">' + parseFloat(item).toFixed(2) + '</td>' : '<td>' + parseFloat(item).toFixed(2) + '</td>');
        }

        index % 5 === 1 ? value = '<td>' + parseFloat(item).toFixed(2) + '</td>' : '';
        index % 5 === 2 ? value2 = '<td>' + parseFloat(item).toFixed(2) + '</td>' : '';
        index % 5 === 3 ? value3 = (parseFloat(item) < 0 ? '<td class="text-danger">' + parseFloat(item).toFixed(2) + '</td>' :  parseFloat(item) > 0 ? '<td class="text-success">' + parseFloat(item).toFixed(2) + '</td>' : '<td>' + item + '</td>') : '';

        index % 5 === 4 ? res += '<tr>' + game + value.replace(/\d(?=(\d{3})+\.)/g, '$&,') + value2.replace(/\d(?=(\d{3})+\.)/g, '$&,') + value3.replace(/\d(?=(\d{3})+\.)/g, '$&,') + value4.replace(/\d(?=(\d{3})+\.)/g, '$&,') + '</tr>' : '';
    });

    tbl = '<table class="nowrap table table-sm table-bordered table-hover responseTable">';
    tbl += '<thead><tr>'
    tbl += '<th><?=lang('Label.games');?></th>';
    tbl += '<th><?=lang('Label.ownbet');?></th>';
    tbl += '<th><?=lang('Label.owneffectbet');?></th>';
    tbl += '<th><?=lang('Label.ownwin');?></th>';
    tbl += '<th><?=lang('Label.ownwinlose');?></th>';
    tbl += '</tr></thead>';
    tbl += '<tbody>' + res + '</tbody>';
    tbl += '</table>';
    return tbl;
...
function format(d)
{
    ...
    obj.forEach(function(item, index) {
        if( index % 5 === 0 ) {
            ...
            value4 = (parseFloat(item) < 0 ? '<td class="text-danger bg-warning">' + parseFloat(item).toFixed(2) + '</td>' :  parseFloat(item) > 0 ? '<td class="text-success bg-warning">' + parseFloat(item).toFixed(2) + '</td>' : '<td>' + parseFloat(item).toFixed(2) + '</td>');
        }
...
function loadparent()
{
    let settledate = $('.filterForm [name=settledate]').val();
    compTable(settledate,'<?=$parent;?>');
    psTable(settledate,'<?=$parent;?>');
