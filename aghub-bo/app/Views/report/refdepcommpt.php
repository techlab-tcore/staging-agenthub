...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    refcommTable(fromdate,todate,'<?=$parent;?>');
...
function refcommTable(from,to,parent)
{
    const refcommTable = $('#refcommTable').DataTable({
        ...
        ajax: {
            type : "POST",
            url: '/list/referral-deposit-commission/position-taking',
        ...
        drawCallback: function(oSettings, json) {
            $('#refcommTable tbody tr').find('td').not('td:first-child,td:nth-child(2),td:nth-child(3)').addClass('text-end');
