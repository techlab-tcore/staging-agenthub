<section class="card border-light shadow">
    ...
    <div class="card-body">
        <article class="card-text p-3">
            ...
            <table id="agcommTable" class="w-100 nowrap table table-sm table-bordered table-hover">
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const agcommTable = $('#agcommTable').DataTable({
        ...
        ajax: function(data, callback, settings) {
            ...
            $.ajax({
                url: '/list/report/agent-commission',
                type: 'post',
                data: payload,
        ...
        footerCallback: function ( row, data, start, end, display ) {
            ...
            $(api.column(5).footer()).html(parseFloat(subtotal).toFixed(5).replace(/(\.\d{4})\d*/, "$1").replace(/(\d)(?=(\d{5})+\b)/g, "$1,"));
        },
        drawCallback: function(oSettings, json) {
            $('#agcommTable tbody tr').find('td:nth-child(6)').addClass('text-end');
            $('#agcommTable tbody tr td.dataTables_empty').removeClass('text-end');
        },
        aoColumnDefs: [{
            aTargets: [5],
    ...
    $('#agcommTable tbody').on('click', 'td.details-control', function() {
        var tr = $(this).closest('tr');
        var row = agcommTable.row( tr );
        ...
        if(row.child.isShown()) {
            ...
            // Open this row
            row.child(format_settle(row.data())).show();
            tr.addClass('shown');
        }
    });
...
function format_settle(d)
{
    let obj = String(d).split(',').slice(5);

    var tbl='', res='', game='', value='', value2='', value3='', value4='', value5='';
    obj.forEach(function(item, index) {
        index % 6 === 0 ? game = '<td>' + item + '</td>' : value5 = '<td>' + parseFloat(item).toFixed(5).replace(/(\.\d{4})\d*/, "$1").replace(/(\d)(?=(\d{5})+\b)/g, "$1,") + '</td>';
        index % 6 === 1 ? value = '<td>' + item + '</td>' : '';
        index % 6 === 2 ? value2 = '<td>' + parseFloat(item).toFixed(5).replace(/(\.\d{2})\d*/, "$1").replace(/(\d)(?=(\d{3})+\b)/g, "$1,") + '</td>' : '';
        index % 6 === 3 ? value3 = '<td>' + parseFloat(item).toFixed(5).replace(/(\.\d{2})\d*/, "$1").replace(/(\d)(?=(\d{3})+\b)/g, "$1,") + '</td>' : '';
        index % 6 === 4 ? value4 = '<td>' + item + '</td>' : '';
        index % 6 === 5 ? res += '<tr>' + game + value + value2 + value3 + value4 + value5 + '</tr>' : '';

    });

    tbl = '<table class="w-auto nowrap table table-sm table-bordered table-hover m-0">';
    tbl += '<thead class="bg-light"><tr>';
    tbl += '<th><?=lang('Input.turnovercount');?></th>';
    tbl += '<th><?=lang('Label.games');?></th>';
    tbl += '<th><?=lang('Label.turnover');?></th>';
    tbl += '<th><?=lang('Label.effectbet');?></th>';
    tbl += '<th><?=lang('Input.commission');?>%</th>';
    tbl += '<th><?=lang('Input.amount');?></th>';
    tbl += '</tr></thead>';
    tbl += '<tbody>' + res + '</tbody>';
    tbl += '</table>';
    return tbl;
