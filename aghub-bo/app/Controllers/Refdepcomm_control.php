...
class Refdepcomm_control extends BaseController
{
    /*
    History
    */

    public function refCommHistoryList()
    {
        ...
        $payload = $this->refdepcomm_model->selectAllRefCommHistory([
    ...
    */
    ...
    public function modifyRefCommPt()
    {
        if( !session()->get('logged_in') ): return false; endif;

        $payload = [
            'userid' => base64_decode($this->request->getpost('params')['uid']),
            'percentage' => (float)$this->request->getpost('params')['refcommpt']
        ];
        $res = $this->refdepcomm_model->updateRefCommPt($payload);
        echo json_encode($res);
    }

    public function minMaxRefCommPt()
    {
        if( !session()->get('logged_in') ): return false; endif;

        $res = $this->refdepcomm_model->selectMinMaxRefCommPt([
            'userid' => base64_decode($this->request->getpost('params')['uid'])
        ]);
        echo json_encode($res);
    }

    public function getRefCommPt()
    {
        if( !session()->get('logged_in') ): return false; endif;

        $res = $this->refdepcomm_model->selectRefCommPt([
            'userid' => base64_decode($this->request->getpost('params')['uid'])
        ]);
        if( $res['code']==1 && $res['data']!=[] ):
            $arr = end($res['data']);
            $refcommPt = $arr['percentage'];
        else:
            $refcommPt = 0;
        endif;
        echo json_encode(['code'=>$res['code'], 'refcommPt'=>$refcommPt]);
    }

    public function refCommPtReport()
    {
        if( !session()->get('logged_in') ): return false; endif;
        ...
        $res = $this->refdepcomm_model->selectAllRefCommPtList([
