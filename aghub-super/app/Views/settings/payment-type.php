<section class="card border-light">
    ...
    <div class="card-body">
        <article class="card-text p-3">
            <table id="paytypeTable" class="w-100 nowrap table table-sm table-bordered">
                <thead>
                <tr>
                <th>Code</th>
                <th>Payment Types</th>
    ...
    <div class="modal-dialog modal-fullscreen-lg-down modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit Payment Types</h5>
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    <label class="d-block">Payment Types</label>
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const paytypeTable = $('#paytypeTable').DataTable({
    ...
    $('.modal-modify form').on('submit', function(e) {
        ...
        if (this.checkValidity() !== false) {
            ...
            $.post('/settings/payment-types/modify', {
                params
            }, function(data, status) {
                const obj = JSON.parse(data);
                if( obj.code == 1 ) {
                    swal.fire("Updated!", obj.message , "success").then(() => { paytypeTable.ajax.reload(null,false); });
                } else if( obj.code==39 ) {
                    forceUserLogout();
                } else {
...
function editPaytype(code, currencycode)
{
    $('.modal-modify').modal('toggle');
    ...
    $.post('/payment-types/get', {
        params
    }, function(data, status) {
        const obj = JSON.parse(data);
        if( obj.code == 1 ) {
            $('.modal-modify [name=currencydode]').val(currencycode);
