<section class="card border-light">
    ...
    <div class="card-body">
        <div class="px-3">
            <button type="button" class="btn btn-primary bg-gradient" data-bs-toggle="modal" data-bs-target="#modal-addcurrency"><i class="las la-plus-circle me-1"></i>Add Currency</button>
        </div>

        <article class="card-text p-3">
            <table id="currencyTable" class="w-100 nowrap table table-sm table-bordered">
                <thead>
                <tr>
                <th>Code</th>
                <th>Currency</th>
...
<section class="modal fade modal-addcurrency" id="modal-addcurrency" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modal-addcurrency" aria-hidden="true">
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    $('.modal-addcurrency form').on('submit', function(e) {
        e.preventDefault();
        ...
        if (this.checkValidity() !== false) {
            ...
            $.post('/settings/currency/add', {
                params
            }, function(data, status) {
                const obj = JSON.parse(data);
                if( obj.code == 1 ) {
                    swal.fire("Updated!", obj.message , "success").then(() => { currencyTable.ajax.reload(null,false); });
                } else if( obj.code==39 ) {
                    forceUserLogout();
                } else {
            ...
            .done(function() {
                $('.modal-addcurrency').modal('toggle');
            })
            .fail(function() {
    ...
    const addcurrencyEvent = document.getElementById('modal-addcurrency');
    addcurrencyEvent.addEventListener('hidden.bs.modal', function (event) {
...
function editCurrency(code,name,currencycode)
{
    $('.modal-modify').modal('toggle');
    $('.modal-modify [name=code]').val(code);
    $('.modal-modify [name=currency]').val(name);
